...
export class UserlistAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<User[]>('https://backend.aventuraevents.site/users').subscribe(
      (response) => {
        this.users = response;
      },
      (error) => {
  ...
  toggleStatus(user: User): void {
    ...
    this.http.put(`https://backend.aventuraevents.site/blocking/${userId }`, user ).subscribe(
      (response) => {
  ...
  logout(): void {
    this.http.post('https://backend.aventuraevents.site/admin/logout', {}).subscribe(() => {
