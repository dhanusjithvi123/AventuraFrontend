...
export class OrganiserRequestComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<User[]>('http://localhost:5000/organisaer/organisaerRequestList').subscribe(
      (response) => {
  ...
  toggleVerify(user: User): void {
    ...
    this.http.put(`http://localhost:5000/organisaer/verfiyconfrom/${userId}`, user).subscribe(
      (response) => {
