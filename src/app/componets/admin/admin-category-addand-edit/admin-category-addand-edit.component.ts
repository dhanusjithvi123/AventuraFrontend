...
export class AdminCategoryAddandEditComponent   implements OnInit  {
  ...
  ngOnInit(): void {
    ...
    this.http.get<category[]>('https://backend.aventuraevents.site/admin/categorylist').subscribe(
      (response) => {
  ...
  CategoryaddingSubmit(): void {
    if (this.categoryForm.valid) {
      ...
      this.http.post('https://backend.aventuraevents.site/admin/Categoryadding', formData)
        .subscribe(
          (response) => {
